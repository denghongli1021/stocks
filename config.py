...
# 三大法人買賣超特徵(外資/投信/自營商,僅台股、資料來源 FinMind)。
# 開啟後台股模型會多 4 個法人特徵;美股無此資料,自動只用技術特徵。
# ⚠️ 實測結論(控制變因,同股同切分同種子):加入每日法人特徵後「方向準確率」反而
#    下降(2330:52%→46%、2454:46%→45%),MAPE 也沒改善。原因是每日法人買賣超對
#    「隔日」報酬的訊號太弱,多 4 個雜訊特徵在有限資料下更易過擬合。
#    因此預設關閉;功能與開關完整保留,可改長預測週期或別的模型再實驗。
